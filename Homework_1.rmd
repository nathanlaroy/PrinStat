...
To arrive at an analytic expression for the MLE of the scale parameter $\sigma$, we proceed as follows, starting from the log-likelihood function derived earlier:

$$
l(\mu,\sigma,\zeta) = n \log(\zeta) + n \zeta \log(\sigma) - \sum_{n=1}^{n} (1+\zeta) 
\log(x_i-\mu) + \left(\frac{x_i-\mu}{\sigma}\right)^{-\zeta} \space\space,
$$ we calculate the partial derivative of $l(\mu, \sigma, \zeta)$ with respect to $\sigma$.

$$
\begin{aligned}
\frac{\partial l}{\partial \sigma}
&= \frac{\partial}{\partial\sigma} \left(n\log(\zeta) \space + \space n\zeta \log(\sigma) \right)- \frac{\partial}{\partial\sigma}\sum_{i=1}^{n} (1+\zeta) \log(x_i-\mu) + \left( \frac{x_i-\mu}{\sigma}\right)^{-\zeta} = 0\\
...
2)  All terms without $\sigma$ resolve to zero. Derivative of $a\space\cdot\space\log(x)$ is $\frac{a}{x}$.

3)  Chain rule: first derive with respect to $a = \frac{x_i-\mu}{\sigma}$, multiplied by derivative of $a$ with respect to $\sigma$.

4)  Cancel out the negative signs within the summation.

From this result, we isolate $\sigma$: 
$$
\begin{aligned}
...
1)  Set partial derivative of $l(\mu, \sigma, \zeta)$ with respect to $\sigma$ to zero.

2)  Isolate first term containing $\sigma$. Distribute exponent $-\zeta-1$ among numinator and denominator of fraction.

3)  Reassemble factors such that all $\sigma$ and all $(x_i-\mu)$ are together.

4)  Apply product rule on exponents with shared bases for both all $\sigma$ and all $(x_i-\mu)$, and put $x_i$-independent factor in front of summation sign.

5)  Isolate $\sigma$ to left of equation, and $\zeta$ to right, and apply product rule of exponents with shared bases.

6)  Apply product rule of exponents with shared bases on $\sigma$, and raise both sides of the equation to the power of $-1$.
